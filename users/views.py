...
class RegisterView(CreateView):
    ...
    def form_valid(self, form):
        """
        Метод form_valid вызывается для проверки, когда форма валидна и готова к сохранению.
        Сохраняет данные пользователя, устанавливает его аккаунт как неактивный,
        генерирует токен и создает URL для подтверждения адреса электронной почты.
        Отправляет письмо с этой ссылкой на электронную почту пользователя
        """
...
class UserConfirmEmailView(View):
    ...
    def get(self, request, uidb64, token):
        """
        Метод get() обрабатывает GET-запрос, который содержит параметры uidb64 и token.
        Проверяет параметры uidb64 и token, и если они действительны, активирует аккаунт пользователя,
        выполняет вход в систему и перенаправляет на страницу подтверждения или возвращает ошибку
        """
...
class UserListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        """
        Переопределяем метод get_context_data(), чтобы добавить дополнительный
        контекст teacher_list, содержащий только преподавателей (User с is_staff=True).
        Далее эта контекстная переменная teacher_list используется в шаблоне
        """
...
class UserUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_object(self, queryset=None):
        """
        Метод возвращает текущего пользователя, связанного с запросом,
        который может быть использован в других методах представления для доступа к данным
        или выполнения действий от имени этого пользователя.
...
def set_new_password(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            subject = 'Смена пароля вашего профиля на сайте EPS'
