from exams.models import Answer, QuestionAnswer
...
def calculate_exam_results(request, questions, exam_id):
    ...
    Returns:
        dict: Словарь с результатами экзамена и
        информацией о неправильных ответах.
    """
    ...
    for question in questions:
        selected_answers = request.POST.getlist(
            f"question_{question.id}_answers"
        )
        selected_answers_text = \
            [
                Answer.objects.get(id=answer_id).text
                for answer_id in selected_answers
            ]

        # Проверяем, являются ли выбранные ответы правильными
        is_correct = QuestionAnswer.objects.filter(
            question=question,
    ...
    # Проверка, пройден ли тест при заданном пороге 80%
